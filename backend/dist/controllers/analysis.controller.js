...
class AnalysisController {
     ...
     */
    async getPoliticianAnalysis(req, res, next) {
        try {
            const { id } = req.params;
            const analysis = await scoring_service_1.scoringService.getDetailedAnalysis(id);
            res.json({
                success: true,
                data: analysis,
            });
    ...
    async recalculateScore(req, res, next) {
        try {
            const { id } = req.params;
            const breakdown = await scoring_service_1.scoringService.calculatePoliticianScore(id);
            res.json({
                success: true,
                data: {
                    politicianId: id,
     ...
     */
    async updateAllScores(_req, res, next) {
        try {
            // Run in background
            scoring_service_1.scoringService.updateAllScores().catch(err => {
                logger_1.logger.error('Background score update failed:', err);
            });
            res.json({
                success: true,
     ...
     */
    async getSchedulerStatus(_req, res, next) {
        try {
            const status = scheduler_service_1.schedulerService.getJobStatus();
            res.json({
                success: true,
                data: status,
            });
     ...
     */
    async runScheduledJob(req, res, next) {
        try {
            const { jobName } = req.params;
            // Run in background
            scheduler_service_1.schedulerService.runJob(jobName).catch(err => {
                logger_1.logger.error(`Manual job run failed for ${jobName}:`, err);
            });
            res.json({
                success: true,
     ...
     */
    async getScoringMethodology(_req, res, _next) {
        const methodology = {
     ...
     */
    async comparePoliticians(req, res, next) {
        try {
    ...
    generateComparisonSummary(politicians) {
