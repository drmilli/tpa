...
class ScoringService {
     ...
     */
    async calculatePoliticianScore(politicianId) {
        ...
        const [promiseFulfillment, legislativeActivity, projectCompletion, publicSentiment, mediaPresence, controversyImpact,] = await Promise.all([
        ...
        const totalScore = Math.min(100, Math.max(0, (promiseFulfillment * SCORING_WEIGHTS.promiseFulfillment) +
     ...
     */
    calculatePromiseFulfillment(promises) {
     ...
     */
    calculateLegislativeActivity(bills) {
     ...
     */
    async calculatePublicSentiment(politicianName) {
        try {
            ...
            logger_1.logger.warn(`Failed to calculate sentiment for ${politicianName}:`, error);
     ...
     */
    async calculateMediaPresence(politicianName) {
        try {
            ...
            logger_1.logger.warn(`Failed to calculate media presence for ${politicianName}:`, error);
            return 50;
        }
    }
    /**
     ...
     */
    calculateControversyImpact(controversies) {
     ...
     */
    async analyzeSentimentWithAI(politicianName) {
        try {
            if (!openai) {
                logger_1.logger.warn('OpenAI not configured, using neutral sentiment fallback');
                return 0;
            }
...
Return ONLY a JSON object in this exact format:
{
  ...
  "summary": "<brief explanation>"
            ...
            logger_1.logger.error('AI sentiment analysis failed:', error);
            return 0; // Neutral
        }
    }
    /**
     ...
     */
    async fetchNewsData(politicianName) {
        try {
            ...
            logger_1.logger.error('News fetch failed:', error);
            return {
                totalMentions: 0,
                sentimentScore: 0,
                topTopics: [],
     ...
     */
    async analyzeHeadlinesSentiment(headlines, politicianName) {
     ...
     */
    async getAIBasedNewsAnalysis(politicianName) {
        try {
            if (!openai) {
                        ...
                        content: `Provide news analysis for Nigerian politician "${politicianName}". Return ONLY JSON:
{
  ...
  "recentHeadlines": ["headline1", "headline2"]
    ...
    async updateAllScores() {
        ...
        logger_1.logger.info(`Updating scores for ${politicians.length} politicians...`);
        for (const politician of politicians) {
            try {
                ...
                logger_1.logger.info(`Updated score for ${politician.firstName} ${politician.lastName}: ${breakdown.totalScore}`);
            }
            catch (error) {
                logger_1.logger.error(`Failed to update score for ${politician.id}:`, error);
            }
        }
     ...
     */
    async storeMetrics(politicianId, breakdown) {
            ...
            if (existingMetric) {
                ...
                const newMetric = await prisma.metric.create({
                    data: {
                        ...
                        weight: SCORING_WEIGHTS[metric.name.toLowerCase().replace(' ', '')] || 0.1,
                    },
                });
    ...
    async updateRankings() {
        ...
        logger_1.logger.info('Rankings updated successfully');
    }
    /**
     * Get detailed analysis for a politician
     */
    async getDetailedAnalysis(politicianId) {
        const politician = await prisma.politician.findUnique({
            where: { id: politicianId },
            include: {
                State: true,
        ...
        return {
            politician,
            scoreBreakdown: politician.Score.reduce((acc, s) => {
     ...
     */
    async getAIComprehensiveAnalysis(politician, newsAnalysis) {
        try {
            if (!openai) {
                return {
                    strengths: [],
