...
class EmailService {
    constructor() {
        ...
        this.isConfigured = !!(process.env.EMAIL_HOST &&
            process.env.EMAIL_USER &&
            process.env.EMAIL_PASSWORD);
        if (!this.isConfigured) {
            logger_1.logger.warn('Email service not configured. Email notifications will not be sent.');
        }
    }
    /**
    ...
    async sendEmail(options) {
        if (!this.isConfigured) {
            logger_1.logger.warn(`Email not sent (service not configured): ${options.subject}`);
            return false;
        }
        try {
